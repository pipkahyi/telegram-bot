...
class ProfileStates(StatesGroup):
    waiting_role = State()  # Убрали waiting_name
...
def is_moderator(user_id):
    # ДА, ВЫ МОЖЕТЕ ДОБАВЛЯТЬ ID В СПИСОК MODERATORS
...
async def save_profile(user_id, username, role, fandom, age, city, bio, photo):
    try:
        # Генерируем имя из username или user_id
        if username:
            name = f"@{username}"
        else:
...
async def start_anketa(message: types.Message, state: FSMContext):
    ...
    await state.set_state(ProfileStates.waiting_role)  # НАЧИНАЕМ С РОЛИ
...
async def process_photo(message: types.Message, state: FSMContext):
    try:
        ...
        if success:
            # Генерируем имя для отображения
            display_name = f"@{message.from_user.username}" if message.from_user.username else f"Пользователь {message.from_user.id}"
    ...
    try:
        ...
        async with pool.acquire() as conn:
            profile = await conn.fetchrow(
                "SELECT status, username FROM profiles WHERE user_id = $1",
                user_id
            )
        ...
        # Генерируем имя для уведомления
        display_name = f"@{profile['username']}" if profile['username'] else f"Пользователь {user_id}"
            ...
            if profile:
                ...
                await message.answer_photo(
                    ...
                    caption=f"📋 <b>Ваша анкета:</b>\n\n"
                           f"👤 <b>Пользователь:</b> {profile.get('name', 'Не указано')}\n"
                ...
                for profile in profiles:
                    await message.answer_photo(
                        ...
                        caption=f"🔍 <b>Найдена анкета:</b>\n\n"
                               f"👤 <b>Пользователь:</b> {profile['name']}\n"
                               f"🎭 <b>Роль:</b> {profile['role']}\n"
                               f"📚 <b>Фандом:</b> {profile.get('fandom', 'Не указан')}\n"
                               f"🎂 <b>Возраст:</b> {profile['age']}\n" 
                               f"🏙️ <b>Город:</b> {profile['city']}\n"
        ...
        async with pool.acquire() as conn:
            ...
            if profiles:
                ...
                for i, profile in enumerate(profiles, 1):
                    profile_list += (
                        f"{i}. 👤 <b>Пользователь:</b> {profile['name']}\n"
                        f"   🎭 <b>Роль:</b> {profile['role']}\n"
                        f"   📚 <b>Фандом:</b> {profile.get('fandom', 'Не указан')}\n"
                        f"   🎂 <b>Возраст:</b> {profile['age']}\n"
                        f"   🏙️ <b>Город:</b> {profile['city']}\n"
